...
export default function App() {
  ...
  // Detect Supabase invite and password recovery redirects
  useEffect(()=>{
    ...
    // Store hash before Supabase clears it
    const savedHash = window.location.hash;
    // Listen for auth state changes — catches invite and recovery flows
    const {data:{subscription}} = supabase.auth.onAuthStateChange((event, session)=>{
      if(event==="PASSWORD_RECOVERY") {
        setRecovering(true);
        setView("biz-portal");
      }
      if(event==="SIGNED_IN" && (savedHash.includes("type=invite") || savedHash.includes("type=signup"))) {
        setRecovering(true);
        setView("biz-portal");
      }
    });
