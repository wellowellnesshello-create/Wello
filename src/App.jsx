...
function Card({ biz, onSelect, syncing, saved, onToggleSave }) {
  ...
  return (
    <div onClick={()=>onSelect(biz)} style={{cursor:"pointer"}}
      onMouseEnter={e=>e.currentTarget.querySelector('.card-img').style.transform="scale(1.05)"}
      onMouseLeave={e=>e.currentTarget.querySelector('.card-img').style.transform="scale(1)"}>
      {/* 4:5 image */}
      <div style={{position:"relative",paddingBottom:"125%",borderRadius:12,overflow:"hidden",marginBottom:16,background:"#E4E2DD"}}>
        <img className="card-img" src={biz.img} alt={biz.name}
          style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",transition:"transform .7s ease"}}/>
        {/* Credit badge — glass pill top right */}
        ...
        {/* Save button — glass circle top left */}
        <button onClick={e=>{e.stopPropagation();onToggleSave(biz.id);}}
          style={{position:"absolute",top:12,left:12,width:32,height:32,borderRadius:"50%",background:"rgba(255,255,255,0.92)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:saved?"#e05c5c":"#74796e",transition:"transform .2s"}}
      ...
      <div style={{paddingLeft:2}}>
        ...
        <p style={{fontFamily:F2,fontSize:13,color:"#74796E",margin:"0 0 4px",display:"flex",alignItems:"center",gap:4}}>
          <span style={{fontSize:11}}>📍</span> {biz.loc} · {biz.cat}
        </p>
        {next
    ...
    <div>
      ...
      <section style={{position:"relative",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 24px 60px",overflow:"hidden",background:"linear-gradient(to bottom, #FBF9F4 0%, #FBF9F4 60%, rgba(250,222,192,0.2) 100%)"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:840,width:"100%",textAlign:"center"}}>
          ...
          <p style={{fontFamily:F2,fontSize:18,color:"#74796E",fontWeight:500,lineHeight:1.6,maxWidth:480,margin:"0 auto 48px",letterSpacing:"-0.2px"}}>
      ...
      <section style={{padding:"80px 32px",maxWidth:1200,margin:"0 auto"}}>
        <div style={{display:"flex",flexWrap:"wrap",alignItems:"flex-end",justifyContent:"space-between",marginBottom:52,gap:20}}>
          <div>
            <span style={{fontFamily:F2,fontSize:11,fontWeight:700,color:"#213C18",letterSpacing:"4px",textTransform:"uppercase",display:"block",marginBottom:12}}>Curated Collections</span>
      ...
      {/* ── VALUES SECTION ── */}
      <section style={{background:"#F5F3EE",padding:"80px 32px"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center"}}>
          {/* Left — circular image with floating card */}
          <div style={{position:"relative"}}>
            <div style={{borderRadius:"50%",overflow:"hidden",border:"8px solid rgba(255,255,255,0.5)",boxShadow:"0 25px 60px rgba(0,0,0,0.12)",aspectRatio:"1/1"}}>
              <img src="https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=600&q=80" alt="Wellness" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            </div>
            <div style={{position:"absolute",bottom:-32,right:-32,background:"#fff",padding:"24px",borderRadius:16,boxShadow:"0 20px 40px rgba(0,0,0,0.1)",maxWidth:220}}>
              <div style={{fontSize:32,marginBottom:8}}>🌿</div>
              <h4 style={{fontFamily:F2,fontWeight:700,fontSize:17,color:"#1B1C19",margin:"0 0 6px"}}>Sustainably Minded</h4>
              <p style={{fontFamily:F2,fontSize:13,color:"#74796E",margin:0,lineHeight:1.5}}>We partner with venues that prioritise ethical practices.</p>
            </div>
          </div>
          {/* Right — text */}
          <div>
            <h2 style={{fontFamily:F2,fontSize:"clamp(28px,4vw,44px)",fontWeight:700,color:"#213C18",letterSpacing:"-1.5px",margin:"0 0 32px",lineHeight:1.1}}>The Wellness Pass for Island Living</h2>
            <div style={{display:"flex",flexDirection:"column",gap:24}}>
              {[
                {icon:"🔓",title:"Unlimited Access",desc:"No memberships required. Pure flexible access to the best wellness venues."},
                {icon:"⭐",title:"Vetted Quality",desc:"Every venue is personally visited and verified for quality."},
                {icon:"🌊",title:"Rooted in Community",desc:"We give back to the coastlines, communities and environments we operate in."},
              ].map(({icon,title,desc})=>(
                <div key={title} style={{display:"flex",gap:20,alignItems:"flex-start"}}>
                  <div style={{width:48,height:48,borderRadius:"50%",background:"#FADEC0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{icon}</div>
                  <div>
                    <h4 style={{fontFamily:F2,fontWeight:700,fontSize:17,color:"#1B1C19",margin:"0 0 4px"}}>{title}</h4>
                    <p style={{fontFamily:F2,fontSize:14,color:"#74796E",margin:0,lineHeight:1.6}}>{desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <header style={{display:"flex",flexWrap:"wrap",alignItems:"flex-end",gap:32,marginBottom:48,paddingTop:16}}>
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:8}}>
              <h1 style={{fontFamily:F2,fontSize:"clamp(28px,4vw,44px)",fontWeight:800,color:"#213C18",letterSpacing:"-1.5px",margin:0}}>Jane Smith</h1>
              <span style={{background:"#FADEC0",color:"#766149",padding:"4px 14px",borderRadius:999,fontSize:11,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase"}}>Member</span>
            </div>
            <p style={{fontFamily:F2,fontSize:15,color:"#74796E",margin:"0 0 12px",lineHeight:1.5}}>Exploring wellness across the island.</p>
          ...
          <button onClick={()=>onSetView("credits")}
            style={{background:"#213C18",color:"#fff",border:"none",borderRadius:999,padding:"12px 24px",fontFamily:F2,fontSize:14,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 14px rgba(33,60,24,0.25)",transition:"transform .15s"}}
              ...
              <div style={{padding:"20px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                  <div>
                    ...
                    <p style={{fontFamily:F2,fontSize:12,color:"#74796E",margin:0}}>List your venue and manage integrations.</p>
                  </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:24,marginTop:60}}>
          <div style={{background:"#213C18",color:"#fff",padding:"40px",borderRadius:16,display:"flex",flexDirection:"column",justifyContent:"space-between",minHeight:240}}>
            <div>
              ...
              <p style={{fontFamily:F2,fontSize:14,opacity:0.7,margin:0}}>Keep exploring to build your habit.</p>
            </div>
            <div style={{paddingTop:24}}>
              <div style={{fontFamily:F2,fontSize:52,fontWeight:900,lineHeight:1}}>{bookings.length}</div>
          ...
          <div style={{background:"#E4E2DD",padding:"40px",borderRadius:16,position:"relative",overflow:"hidden"}}>
            <div style={{position:"relative",zIndex:1}}>
              ...
              <button onClick={()=>onSetView("explore")}
                style={{background:"#fff",color:"#213C18",border:"none",borderRadius:999,padding:"12px 24px",fontFamily:F2,fontSize:14,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}}>
                Explore Matches
              </button>
            </div>
    ...
    <div style={{paddingTop:48,paddingBottom:96}}>
        ...
        <div style={{maxWidth:1100,margin:"0 auto",padding:"0 32px",display:"grid",gridTemplateColumns:"1fr 420px",gap:48,alignItems:"start"}}>
          ...
          <div>
            ...
            <div style={{marginBottom:36}}>
              <span style={{fontFamily:F2,fontSize:11,fontWeight:700,color:"#213C18",letterSpacing:"4px",textTransform:"uppercase",display:"block",marginBottom:12}}>Wellness Wallet</span>
              <h1 style={{fontFamily:F2,fontSize:"clamp(28px,4vw,48px)",fontWeight:800,color:"#213C18",letterSpacing:"-2px",margin:"0 0 12px",lineHeight:1}}>Add Wello Credits</h1>
              <p style={{fontFamily:F2,fontSize:16,color:"#74796E",maxWidth:440,lineHeight:1.6,margin:0}}>Credits can be used for any class, gym, spa or adventure. 1 credit = €5 value.</p>
            </div>
            ...
            <div style={{background:"#213C18",borderRadius:16,padding:"24px 28px",marginBottom:24,position:"relative",overflow:"hidden"}}>
              ...
              <p style={{fontFamily:F2,fontSize:11,color:"rgba(255,255,255,0.4)",margin:0}}>10% service fee per booking · max €5</p>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
              {[{icon:"⏱",title:"6 Month Validity",desc:"Credits expire 6 months from purchase."},{icon:"◈",title:"Flexible Booking",desc:"Use across any venue, class or experience."}].map(({icon,title,desc})=>(
            ...
            <div style={{background:"rgba(250,222,192,0.3)",borderRadius:12,padding:"14px 18px"}}>
              <button onClick={()=>setShowPricing(p=>!p)}
                ...
                <span>How credits work · 1 credit = €5 value</span>
                <span>{showPricing?"↑":"↓"}</span>
              </button>
              {showPricing&&(
                <div style={{marginTop:14,paddingTop:14,borderTop:"1px solid rgba(182,142,92,0.2)"}}>
                  {CREDIT_PRICING.map(r=>(
                    <div key={r.cat} style={{display:"grid",gridTemplateColumns:"1.4fr .9fr .9fr",gap:8,padding:"8px 0",borderBottom:"1px solid rgba(182,142,92,0.1)"}}>
                      <div>
                        <p style={{fontFamily:F2,fontSize:12,fontWeight:600,color:"#1B1C19",margin:"0 0 2px"}}>{r.cat}</p>
                        <p style={{fontFamily:F2,fontSize:10,color:"#74796E",margin:0}}>{r.example}</p>
                      </div>
                      <p style={{fontFamily:F2,fontSize:12,color:"#213C18",fontWeight:600,margin:0,alignSelf:"center"}}>{r.offPeak}</p>
                      <p style={{fontFamily:F2,fontSize:12,color:"#74796E",margin:0,alignSelf:"center"}}>{r.peak}</p>
                    </div>
                  ))}
                  <p style={{fontFamily:F2,fontSize:11,color:"#74796E",margin:"10px 0 0",lineHeight:1.5}}>A 10% service fee (max €5) is charged per booking. Credits expire 6 months from purchase.</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:0}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",paddingBottom:16}}>
                  <div>
                    ...
                    <p style={{fontFamily:F2,fontSize:14,fontWeight:600,color:"#213C18",margin:0}}>{customCr} Wello Credits</p>
                  </div>
                  <p style={{fontFamily:F2,fontSize:16,fontWeight:700,color:"#213C18",margin:0}}>€{totalPrice}</p>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 0",borderTop:"1px solid rgba(195,200,188,0.2)",borderBottom:"1px solid rgba(195,200,188,0.2)"}}>
                  <div>
                    <p style={{fontFamily:F2,fontSize:13,color:"#74796E",margin:"0 0 2px"}}>Unit price</p>
                    <p style={{fontFamily:F2,fontSize:12,color:"#213C18",fontStyle:"italic",margin:0}}>1 Credit = €5.00</p>
                  </div>
                </div>
                <div style={{padding:"16px 0 24px",borderBottom:"2px solid rgba(33,60,24,0.05)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                    <span style={{fontFamily:F2,fontSize:18,fontWeight:700,color:"#213C18"}}>Total</span>
                    <div style={{textAlign:"right"}}>
                      <span style={{fontFamily:F2,fontSize:40,fontWeight:800,color:"#213C18",letterSpacing:"-2px"}}>€{totalPrice}</span>
                      <p style={{fontFamily:F2,fontSize:10,letterSpacing:"2px",textTransform:"uppercase",color:"#74796E",margin:"4px 0 0"}}>{customCr} credits</p>
                    </div>
                  </div>
                </div>
                <div style={{paddingTop:24}}>
      ...
      {step===3&&(
        <div style={{maxWidth:420,margin:"0 auto",padding:"0 32px",textAlign:"center",paddingTop:40}}>
          ...
          <p style={{fontFamily:F2,fontSize:15,color:"#74796E",margin:"0 0 4px"}}>◈ {customCr} credits added to your account.</p>
        ...
        <nav style={{position:"fixed",top:0,width:"100%",zIndex:500,background:"rgba(251,249,244,0.85)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid rgba(195,200,188,0.2)"}}>
          <div style={{maxWidth:1200,margin:"0 auto",padding:"0 32px",display:"flex",alignItems:"center",justifyContent:"space-between",height:72}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:20}}>
              <button onClick={()=>setView("profile")} style={{background:"transparent",border:"none",fontSize:20,cursor:"pointer",color:"#213C18",padding:4,transition:"opacity .15s"}}
                onMouseEnter={e=>e.target.style.opacity="0.7"} onMouseLeave={e=>e.target.style.opacity="1"}>🔔</button>
          ...
          <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexWrap:"wrap",justifyContent:"space-between",alignItems:"flex-start",gap:32}}>
            ...
            <div style={{display:"flex",gap:32,flexWrap:"wrap"}}>
              {["Privacy","Terms","Marketplace","Our Mission","Contact"].map(l=>(
                <a key={l} style={{fontFamily:"'Manrope',system-ui,sans-serif",fontSize:13,color:"#43483F",cursor:"pointer",opacity:0.8,textDecoration:"none",transition:"opacity .15s"}}
                  onMouseEnter={e=>e.target.style.opacity="1"} onMouseLeave={e=>e.target.style.opacity="0.8"}>{l}</a>
              ))}
            </div>
            <div>
