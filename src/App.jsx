...
<<<<<<< HEAD
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1.4fr .9fr .9fr",gap:8,padding:"0 0 8px",borderBottom:"1px solid rgba(182,142,92,0.15)"}}>
                    ...
                    <p style={{fontFamily:F2,fontSize:10,fontWeight:700,color:"#213C18",margin:0,textTransform:"uppercase",letterSpacing:"1px"}}>Standard</p>
                    <p style={{fontFamily:F2,fontSize:10,fontWeight:700,color:"#B8925C",margin:0,textTransform:"uppercase",letterSpacing:"1px"}}>Peak 7–9am</p>
                  </div>
                  {CREDIT_PRICING.map(r=>(
                    <div key={r.cat} style={{display:"grid",gridTemplateColumns:"1.4fr .9fr .9fr",gap:8,padding:"8px 0",borderBottom:"1px solid rgba(182,142,92,0.1)"}}>
                      ...
                      <p style={{fontFamily:F2,fontSize:12,color:"#B8925C",fontWeight:600,margin:0,alignSelf:"center"}}>{r.peak}</p>
                    </div>
                  ))}
                  {/* Live price range from current listings */}
                  {listings.length > 0 && (()=>{
                    const min = Math.min(...listings.map(l=>l.cr));
                    const max = Math.max(...listings.map(l=>l.cr));
                    return (
                      <div style={{marginTop:10,padding:"8px 10px",background:"rgba(33,60,24,0.05)",borderRadius:8,display:"flex",alignItems:"center",gap:6}}>
                        <span style={{width:6,height:6,borderRadius:"50%",background:"#4ade80",display:"inline-block",flexShrink:0}}/>
                        <p style={{fontFamily:F2,fontSize:11,color:"#213C18",margin:0,fontWeight:600}}>Live on Wello right now: ◈{min}–◈{max} per session</p>
                      </div>
                    );
                  })()}
...
<<<<<<< HEAD
      {/* ── DEMO BANNER ── */}
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:1001,background:"#213C18",padding:"7px 16px",display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
          ...
          <span style={{fontFamily:"'Manrope',system-ui,sans-serif",fontSize:11,color:"rgba(255,255,255,0.7)"}}>Interested in partnering?</span>
          <a href="mailto:hello@wello-wellness.com" style={{fontFamily:"'Manrope',system-ui,sans-serif",fontSize:11,fontWeight:700,color:"#CAECBA",textDecoration:"none",letterSpacing:"0.2px"}}>hello@wello-wellness.com</a>
        </div>
